...
def chang_skill_format() -> list[str]:
    runic_skills = [
...
def generate_character_attributes() -> dict[str, int]:
    character_attribute = {
        atr: random.randint(3, 18)
        for atr in ["strength", "agility", "endurance", "intelligence", "luck"]
        }
    return character_attribute


def cards_attributes() -> tuple[list[str], str, int]:
    people_characteristic = [
        FAKE.first_name(), FAKE.last_name(), FAKE.job(), FAKE.city()
        ]
    attributes = generate_character_attributes()
    skills = random.sample(chang_skill_format(), 3)
    return (
        people_characteristic, attributes['strength'],
        attributes['agility'], attributes['endurance'],
        attributes['intelligence'], attributes['luck'],
        skills
        )


def main() -> None:
    if not os.path.isdir('Card_players'):
        os.makedirs('Card_players')
    for i in range(1, 11):
